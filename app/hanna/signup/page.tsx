...
import { Mail, Lock, User, Eye, EyeOff, ArrowRight, Loader2, Gift, Check, Crown, MessageSquare } from 'lucide-react'
...
function SignupForm() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      const { error: signUpError } = await supabase.auth.signUp({
        ...
        options: {
          data: {
            ...
            pending_coupon_code: couponValid && formData.couponCode
      ...
      router.push(`/hanna/signup/success?plan=${selectedPlan}`)
    } catch {
  ...
  return (
    <>
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-white/60">Elige tu plan y comienza</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        >
          <Crown className="w-5 h-5 text-[#C7517E] mb-2" />
          <h3 className="text-white font-semibold text-sm">Pro</h3>
          <p className="text-white/50 text-xs mt-1">Mensajes ilimitados</p>
          <p className="text-white font-bold mt-2">$19<span className="text-white/50 text-xs font-normal">/mes</span></p>
          {selectedPlan === 'pro' && (
            <div className="absolute top-2 right-2">
              <Check className="w-4 h-4 text-[#C7517E]" />
            </div>
          )}
        </button>
      </div>
      ...
      {selectedPlan === 'pro' && (
        <motion.div
          ...
          className="bg-[#C7517E]/10 border border-[#C7517E]/30 rounded-xl px-4 py-3 mb-6 text-sm text-white/80"
        >
          Después de confirmar tu email, serás redirigida al checkout para activar tu plan Pro.
        </motion.div>
      )}
      ...
      <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 border border-white/20">
        <form onSubmit={handleSignup} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className={`w-full py-3 px-4 font-semibold rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-6 ${
              selectedPlan === 'pro'
                ? 'bg-gradient-to-r from-[#C7517E] to-[#b8456f] text-white hover:from-[#d4608d] hover:to-[#C7517E]'
                : 'bg-gradient-to-r from-[#2CB6D7] to-[#1a9ab5] text-white hover:from-[#36c4e6] hover:to-[#2CB6D7]'
            }`}
          >
            {isLoading ? (
              <>
              ...
              <>
                {selectedPlan === 'pro' ? 'Crear Cuenta Pro' : 'Crear Cuenta Gratis'}
