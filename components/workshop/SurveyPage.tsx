...
export default function SurveyPage() {
  ...
  async function handleSubmit() {
    ...
    // Open Google Review tab BEFORE fetch to avoid popup blocker
    // Browsers only allow window.open in direct response to user click
    let googleTab: Window | null = null
    if (formData.googleRating === 5) {
      const googleUrl = process.env.NEXT_PUBLIC_GOOGLE_REVIEW_URL
      if (googleUrl) {
        googleTab = window.open(googleUrl, '_blank')
      }
    }
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        ...
        // Close the Google tab if survey failed
        if (googleTab && !googleTab.closed) googleTab.close()
        return
      }
  ...
  // Track Google Review click and update DB
  async function handleGoogleReviewClick(surveyEmail: string) {
    try {
      await fetch('/api/workshop/survey/google-click', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: surveyEmail }),
      })
    } catch {
            ...
            {formData.googleRating === 5 && (
              <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-6 text-center border-2 border-blue-200">
                <div className="flex items-center justify-center gap-2 mb-3">
                  ...
                  <p className="font-bold text-gray-800">Un ultimo favor!</p>
                </div>
                <p className="text-gray-600 text-sm mb-4">
                  Tu resena en Google ayuda a que otras emprendedoras nos encuentren.
                ...
                <a
                  ...
                  onClick={() => handleGoogleReviewClick(formData.email)}
                  className="inline-flex items-center gap-2 bg-blue-600 text-white font-semibold py-3 px-8 rounded-xl hover:bg-blue-700 transition-colors shadow-md shadow-blue-200"
                >
